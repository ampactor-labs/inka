...
- **`Hβ.diag.refusal-law-per-class` — THE COLUMN'S SPINE.** An executable
  ...
  **E_MissingModule** (its silent skip in driver_compile_entry is fixed;
  ...
  FELLED 2026-07-17, census 2266 → 615): E_TypeMismatch 1286 → 109 (bare-List +
  alias-forward-ref + bare-parameterized-arity) · E_OwnershipViolation 508 → 37
  (check_ref_escape deleted; the affine branch/scope fix — residue is
  move-vs-borrow-by-callee) · E_OccursCheck 118 → 115 · E_MissingVariable 118 →
