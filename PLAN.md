...
- **3.2 · `mentl where`** — the derived-badge projection (schedule, repr width,
  ...
  original `><` said *parallel HERE*, at the site; the reframe correctly moved
  execution strategy to a `~> Schedule` handler and gave nothing back. The Mentl
  answer is not to duplicate the fact in the surface but to project it —
  `>< [Thread ×4]`, output not input.
