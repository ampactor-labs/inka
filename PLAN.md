...
> **▶ REGIONS ARE LIVE — every fn body is a region, the return is a
> TRANSFER, validated Hylo-quiet on the wheel's own compile (2026-07-16,
> boot re-pinned 94e449dc…, 71 micros, frontier 47/0 + proof-exactness 9/9
> held).** The §5.O layer-3 analysis substrate goes from dormant scaffold
> to real: infer_fn brackets every body in region_enter/exit (ids from 1 —
> 0 stays the global sentinel; nested decls nest; the dormancy had hidden
> a wrong-end stack, §9's front-push-meets-drop_last class, fixed to
> push-at-END); check_escape reads three verdicts — OWN region → the
> RETURN TRANSFER (the value re-tags into the parent;
> Hβ.own.region-return-transfer, result-region-as-parameter — the MLKit
> form read off the medium's own return edge), outer/global → already
> outlives, dead → the genuine escape with both spans. MEASURED: the wheel
> self-compiles under live regions with ZERO escapes — the Hylo-quiet bar
> (§4⑤) on ~37k lines: every legal return transfers, no false flags;
> mn-region-transfer=5 (nested two-level construct-return-destructure)
> enters the baseline. Soundness note traced before the byte: tags key AST
> construction nodes and cross-fn value flow rides untagged CALL nodes, so
> a transferred composite's stale inner field-tags can never re-check
> falsely; the class-aware tag lookup (a block body unifies with its tail
> — the tag misses the class-mate) is the arena increment's named burden.
> WHAT THIS UNBLOCKS: the emit_memory_arena swap
> (Hβ.infer.region-on-tee-alloc-absorb's cash-out) — the region drop IS
> the arena reset, the 4GB never-free image collapses to the live set,
> `!Alloc` after a reset becomes provable, and persist-as-memcpy composes
> per-slab — the analysis it resets by is now real and validated. Named
> next beside it: mutation-site checks need result/argument region
> POLYMORPHISM first (a naive store-check would false-flood the wheel's
> ref-buffer idiom — Tofte-Talpin's actual lesson), and the arm-resume
> transfer joins the same family.**
>
> **▶ THE BOARD IS WHOLE — frontier 47/0, proof-exactness 9/9, census
> 3,002 (2026-07-16, boot re-pinned 4b7f998f…).** The last red class was a
> SILENT WRONG-OFFSET READ, pinned in the binary (never a wheel-eprint —
> the one eprint probe Heisenberg'd to a null, the ⟲ law re-paid):
> $op_region_tracker_check_escape's `tag.region_id` emitted
> `(i32.load offset=0)` — the tag record crossed the type-erased bucket
> scan, the field read resolved to the HANDLE field, and every diagnostic's
> "region N" was the value's own handle (the six ids monotone with line
> number — the tell). Population = exactly fns whose body IS a bare
> tuple/record literal (the only shape where check_escape's body_handle
> carries a tag): the wheel's own 23, the shadow's 5, the two scheduled
> reds — ONE class, all false. THE FIX is the render_audit precedent: the
> tag is the TUPLE (install_span, region_id) destructured positionally
> (provable with no type flow), the dead handle field deleted (zero
> readers — it duplicated the bucket key, the Universal Audit), the
> diagnostic gains the region's install span. The eight interrogations'
> generative half, named where the layer-3 arena lands: a region is the
> ownership aspect of the alloc edge (the side-index dissolves, the
> esc-row-on-node arc); regions open at Alloc-absorbing installs
> (Hβ.infer.region-on-tee-alloc-absorb); the fn RETURN is a TRANSFER into
> the caller's region (Hβ.own.region-return-transfer — result-region-as-
> parameter, the MLKit form), so the region drop IS the arena reset,
> persist-as-memcpy composes per-slab, and E_RegionEscape means one true
> thing: a borrowed pointer whose region died holding it, with the
> two-span Why. Measured: int-tuple repro 1→0, wheel self-compile escapes
> 23→0, shadow 66→61 (fingerprint f725d0f6…), frontier 45/2 → 47/0 and
> proof-exactness 9/9 BOTH at the default boot tier; 70/70 micros, march
> ✓✓ FIXED POINT (479,560 lines), IDE re-packed. Every gate in the repo is
> green.**
>
...
> `-> Seven with !Network = ??` the vocabulary enumerator surfaces exactly
> `pure_seven()` with its Reason; direct/transitive/higher-order Network
> offenders are refused through row_subsumes (the crown's own gate) with
> every rejection's Why RETAINED on the projection; the exact patch lands;
> zero debt; exit 7. `Hβ.synth.vocabulary-enumeration` is REAL. The wiring
> truths: the allowed row is the ENCLOSING decl's AUTHORED signed set (the
> published scheme row is the MEET — the negation deliberately dies at
> publication, so the FnStmt's effs are `!E`'s one live home at a decl);
> parse_fn mints decls with JOINED spans (SYNTAX §Parser obligations — a
> start-point span made every decl invisible to containment); the reach
> exclusion reads the weave directly. refinement_admits admits a
> same-refined-alias call by TYPED IDENTITY, the proof layering stated
> exactly (alias proves the declaration; the callee's ledger owns the
> discharge; a refuted callee fails check; R5 owns executable refusal).
> THEN the gate's differential accounting surfaced a wheel-wide phantom:
> every block-let's own STATEMENT node sat NFree (infer typed the value and
> pattern, never the stmt; lower keys LLet by it; two emit width-readers
> ask it) — the stmt now BINDS its value's var, one union-find edge: wheel
> census 34,028 → 31,414, the frontier runtime shadow 320 → 248, the
> fingerprint re-pinned to the smaller truth twice (the gate's documented
> explicit-baseline rule — tooling updates with the times). Designed
> against a 9-agent ultracode fleet; the refuters caught the unmigrated
> teach_synthesize consumer and the carried-proof overclaim; the eight
> interrogations then caught four MORE defects in that ten-line fn (the
> row re-derivation, the pair contract, a hidden first-wins ranking
> violating the tie-break law, the dropped Reason). Named next: the
> scheduled compile legs' inherited 0:0 debt (6 reds, one class), R5's
> executable refusal (proof-exactness 0/7), the multi-survivor tie
> fixture (`io_seven with IO` must be ADMITTED under !Network — the gate's
> next sharpening), `Hβ.synth.vocabulary-arg-holes`,
> `Hβ.synth.vocabulary-reach-index`, `Hβ.cursor.enclosing-decl-edge`.**
>
...
>
> **▶ THE FELT CLI IS CONNECTED — argv READ, the verbs dispatch and WORK (2026-07-14, 91755d6 + f1b13e2).** The audit thesis ("machinery real, performance absent") proven exactly at the L6 surface. `main(argv)` took params `emit_start` filled with ZEROED words, so every invocation was the argv-blind compile-stdin form and the whole verb vocabulary (compile/check/run/audit/query/teach/edit/doc/test/new) was UNREACHABLE — the felt surface COMPILED with floors and pointer-eq, dead until argv woke it. `emit_start` now emits the WASI argv wire (`args_sizes_get`/`args_get` join `is_wasi_import_op` + `wasi_import_reprs`; io.mn grows `wasi_args`/`cstr_to_string`), so `mentl <verb> <path>` reads argv → dispatches → resolves the module → fs-loads it → runs the verb. MEASURED through `boot/mentl.wasm`: **`compile` emits `(module …`; `check` type-checks; `audit` projects the capability set + severance (`greet : … severable: Network + IO + Alloc — unlocks Sandbox …`); `teach` narrates the gradient (`greet: → add with !Alloc to unlock Real-time safe`); `query` answers why/type/effects/ownership** — self-compile byte-exact (469153 lines). Two commits: the argv wire is a TRANSITION (m3==m4, boot re-pinned bea3692b…); the analysis-verb routing is byte-identical (m2==m3, boot re-pinned c4bdba19… to carry the working verbs). SIX latent bugs the connect EXPOSED in never-run code, each fixed at root: `path_to_module` off-by-one (`lib/` stripped as 5 bytes, is 4 — "tutorial"→"utorial"); `QWhy` returned the raw reason (→ `why_expand`, the Fresh-chain walked to root); the three analysis verbs ran `name |> frontend`, parsing the module NAME as source (→ `driver_check_entry` under the full check chain — affine_ledger/region_tracker/arm_state_ctx, the set the deps' inference needs); the render printed nothing (→ `print_string`); `classify_query_verb`'s `w` was pointer-eq so every query fell to QUnknown (→ `w: String`); `split`'s slice-VIEW met the flat list-pattern (→ `list_to_flat`); `render_audit` read record fields in a map lambda where the element type doesn't flow, flooring "field offset unprovable" (→ a TUPLE — positional destructure is provable with no type flow, and the wheel lowers no nominal record PATTERN, `let Type{f}=e` binds f as an undefined global). Named residue: some tutorial fns' rows render as a raw row-var (`r25436@e11623`, `show_effrow` on an unresolved row — `Hβ.cli.audit-row-var-render`, cosmetic); `edit`/`doc`/`test`/`new`/`repl`/`--with` dispatch but their transports (IDE runtime, doc render, test harness, the `Hβ.cli.user-defined-entry-handler-resolution` install) are the L6/band-M peers, next.
>
> **▶ THE PROFILER FOUND THE SILVER BULLET — the resume-cardinality classifier was ~half the compile (2026-07-13, 1400s → 749s).** Host `perf` (surviving `proc_exit`, where `--profile=guest` wrote empty ×3 — §8) pinned `list_index` at 98% of a 5-min sample, 74% under `classify_grade_all`: the classifier's dataflow fixpoint was O(rounds × (N² + calls×N)) — `usage_summary_lookup` an O(N) by-name linear scan per call-node per round, `decls`/entries cons-lists re-indexed O(N²), all × rounds. FIX (infer.mn, the SAME str_hash index the env got): `summary_of` O(1) via `build_summary_index`/`summary_index_get`, `decls`+entries flattened; `usage_summary_lookup` kept for the COLD readers (`resume_summaries_ctx` per-arm, lower.mn:1012 per-fn) — the over-deletion that broke the first build, caught by tracing the edges (never delete a fn without greping its callers). **MEASURED: m3-gen 1400s → 749s (~2×), m2==m3 byte-identical (467908 lines), 66/66 micros, 5/5 crown.** The 98% was of the SAMPLE (the classifier phase); ~47% of the whole — the remaining 749s is the next frontier (perf the fixed m2). **LESSON: the 8-agent code-reading diagnosis (§5.O) MISSED the classifier entirely** — it estimated env/instantiate/LowIR; the env O(n²) I fixed first was 0.5–4% (below noise), the classifier was ~47%. Empirical `perf` beats code-reading estimates; measure the hot path, never guess it. **ITERATION 2 (region tracker, 749s → 490s, 2026-07-14):** `perf` on the fixed m2 pinned `list_index` STILL 98% self-time, now under `op_region_tracker_tag_alloc_join`/`check_escape` (62%+31%) — `lookup_tag` was a linear `find` over the `tagged_values` cons-list per alloc / per composite field, O(N²). FIX (own.mn): `tagged_values` → a 65536-bucket **handle-index** (`i32.eq`, no str_eq — the key is a dense Int); m3-gen **749s → 490s**, m2==m3 byte-identical (468046 lines), `E_RegionEscape` stable at 23 (region correctness lives in stderr not WAT, so verified by the census — preserved), 66/66 micros, 5/5 crown. CUMULATIVE: **1400s → 749s → 490s (~2.9×)** across two perf-found O(1) fixes, each byte-identical. **ITERATION 3 (esc / base_index, 490s → 226s, 2026-07-14):** `perf` pinned `compute_escaping_rows → resolve_callee_indices → base_index_find` at 85% — an O(N) name scan `map`ped over every base = O(N² × callees) (the `esc_assoc` §5.O named). FIX (lower.mn): a str_hash name→position index built once in `indexed_bases`, O(1) resolve; m3-gen **490s → 226s**, m2==m3 byte-identical (468230 lines — escaping rows drive ev-dispatch, so byte-identity IS the correctness check here), 66/66 micros, 5/5 crown. **ITERATION 4 (esc READ-side, 226s → 56s, 2026-07-14):** iteration 3 fixed the WRITE side (`compute_escaping_rows`'s callee-name→base-position resolution, `resolve_callee_indices`); the READ side stayed O(n²) — `evidence_effects_of`/`can_yield` receive `callee_handle`, DISCARD it, extract the name (`callee_name_of_handle`), and linear-scan the `esc_rows` name→row assoc-list per CALL SITE (`escaping_row → ls_escaping_of → esc_assoc`), perf-pinned at 72% of the post-esc-setup compile. FIX (lower.mn, the SAME str_hash index): `esc_rows` is now a 4096-bucket name→row index built ONCE at `ls_register_escaping` (`build_esc_index`), read O(1) at every call site (`esc_index_get`); the handler state inits to `make_list(4096)` (a pre-register lookup returns `None`, never an OOB), reverse-inserted so a name's first match is byte-identical. m3-gen **226s → 56s** (a 4× single-iteration drop — the read side was the dominant remainder), m2==m3 byte-identical (468421 lines — escaping rows drive ev-slot dispatch, so byte-identity IS the correctness check), 8/8 rungs, 66/66 micros, 5/5 crown. The str_hash index is the O(1) WAYPOINT; the deletion-ultimate is `Hβ.lower.esc-row-on-node` (§5.O layer 2 — the escaping row an EDGE on the callee's node, read by following `callee_handle`, the side-table and the name-extraction GONE), generalized by `Hβ.perf.name-is-handle` (layer 1, intern names to handles at lex → the whole str_hash-index class becomes `i32.eq` or dissolves). **CUMULATIVE (iter 1–4): 1400s → 749s → 490s → 226s → 56s (~25×)** across four perf-found O(1) fixes, EVERY step byte-identical. **ITERATION 5** perf of the 56s m2 pinned REACHABILITY at 79% (`reachable_from_main → reach_grow → reach_decl_refs` — a per-frontier-name LINEAR SCAN of the whole lowered decl list, 58.66%; + `reach_has` membership 16.54%) — the name-scan class was NOT exhausted. **ITERATION 6 (reachability O(n²)→O(1) index + the LSuspend Int-op_h-as-name root fix, 56s → 13s, a TRANSITION, 2026-07-14):** replacing `reach_decl_refs` with `build_reach_index` (one pass) + `reach_index_get` (O(1) str_hash) HUNG m2 — and an **ultracode adversarial Workflow (4 code-readers + a binary-patch ground-truth prober) plus the orchestrator's own ⟲ probe** converged on a PRE-EXISTING latent bug the index EXPOSED: `reach_names_expr`'s LSuspend arm prepended the Int `op_h` field as a reachable NAME (`LSuspend(Int, Int, …)`). `reach_decl_refs`'s `str_eq` tolerated the raw Int (length short-circuit); the index's `str_hash` reads it as a string pointer (op_h=72051 → data-section text " (local $nc " → byte_len 0x6f6c2820 = 1.87 GB) and grinds 1.87 billion garbage bytes → the hang. FIX: drop `[o]` (op_h is an Int op-handle; the callee is already reached via `f`). It killed the hang AND made reach MORE PRECISE — boot's garbage-tolerant scan spuriously kept the dead `$println` (its only callers are `test.mn`'s main-unreachable harness), which the leaner reach drops (the m2/m3 diff is that one fn + shifted fn-indices). m3-gen **56s → 13s (4.3×)**; a TRANSITION (m2 = boot's old-emit with the spurious fn, m3 = the new precise reach), **m3 == m4 byte-identical (468699 lines)**, battery 8/8 rungs + 66/66 micros + 5/5 crown verified THROUGH m3, boot RE-PINNED (349a3302…, boot/PROVENANCE.md), IDE re-packed. **CUMULATIVE: 1400s → 749s → 490s → 226s → 56s → 13s (~108×)** across five perf-found O(1) fixes — the ~23-minute self-compile is now **13 seconds**. The reachability `reach_has` membership (16.54% of the old 56s) is still O(n²) — the full O(N) seen-set restructure that dissolves it is UNBLOCKED now that the LSuspend garbage-Int is gone (iteration 6 first attempt with the seen-set hung on the SAME garbage, not a "boot mystery"); named `Hβ.lower.reach-membership-o1`. The deletion-ultimate is `Hβ.lower.reach-edge-on-node` (follow the resolved caller→callee edge, no name lookup — the Carried-Truth form the eight interrogations point to), generalized by `Hβ.perf.name-is-handle`. **ITERATION 7 (iterate flattens once, 13s → 10s, BYTE-IDENTICAL, 2026-07-14):** perf of the 13s m2 pinned `list_index` at 48.85% self-time under `iterate_from` — the map/fold/filter driver's `xs[i]` (`list_index(xs, i)`) is O(i) on the snoc/concat lists those very ops BUILD via `push`/`++` (tags 1/3), so every iteration was O(N²). FIX (prelude.mn): `iterate` flattens once — `iterate_from(list_to_flat(xs), 0, len(xs))`; `list_to_flat` no-ops on an already-flat list, and the flat copy preserves order, so every yield is byte-identical (m2==m3 FIXED POINT, no transition). m3-gen 13s → 10s; the build loop is now ~10–13s/generation (boot IS the fast compiler), so iterating is CHEAP. `list_index` dropped 48.85% → 35.51% but is STILL the residual dominant cost (more O(N²) list_index beyond iterate_from — the next perf). THE LOOP — `perf` the fixed m2 → the next dominant cost → the O(1)/edge/arena/parallel fix → measure → commit — remains the cursor; the residual `list_index` is the next target.
