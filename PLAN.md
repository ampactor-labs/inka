...
## §7 · Current state (grounded 2026-07-08 — **m3 DOWN TO ONE ASSEMBLY ERROR, and the last is a distinct inference root. The f64 REGISTRY was DELETED (the Carried-Truth fix): cfbdf8e's premise — "the wheel trusts the FLOORED inference type" — was REFUTED by the artifact. The `$w.f64` USE node proved `repr_of(lookup_ty)` holds the Float type LIVE; the fn-result-repr registry was a side-ledger RE-DERIVING what the graph already proves (the canonical §2 violation), and it was INERT anyway (the effectful `tail_expr_repr` floored in its own fixpoint — the ev-seam). The isolation micro (`let w = weight(3)` where weight returns Float via arithmetic, so `tail_expr_repr(call)`=registry=RI32 and `repr_join(_,RI32)` reveals `repr_of` ALONE) went `$w` (i32) → `$w.f64` after `call_result_repr = repr_of(lookup_ty(result_h))`, and ASSEMBLED. So the whole registry — the `FnResultRegistry` effect, the `fn_result_registry` handler, `fn_result_lookup`, `seed_fn_result_registry`+`collect_top_fn_result_tails`+`seed_floor_reprs`+`fix_fn_result`+`recompute_fn_reprs`+`reprs_list_equal`, the 3 pipeline installs, the emit_module call — was DELETED; every call/ft width now reads `repr_of(lookup_ty(call_node))` live. MEASURED: m2 103261→102956 lines (LESS code), rungs 8/0, micros-through-m2 45/0, verify ok (NO regression); m3 9 masked f64/i32 mismatches → **ONE**. THE LAST ERROR — `score_one_position` (cursor.mn): `float_of_int(gates) * proximity` emits `f64.mul [f64, i32]` because `proximity = caret_proximity_weight(...)` and caret_proximity_weight is defined LATER (line 280 > 252) — a FORWARD-REFERENCE float call whose result floored at inference. REPRODUCED (`.build/probe/ffwd.mn`: `caller` calls later-defined `helper`, both funcs `(result f64)`, the forward call floors to `f64.mul[f64,i32]`; the BACKWARD `main`→`caller` gives `$r.f64`). This is `Hβ.infer.scc-ordered-walk` — a distinct INFERENCE root, NOT emit. MECHANISM CONFIRMED by micro: `caller(x) = helper(x)` (BARE, no arith) with helper defined AFTER → `caller` emits `(result i32)` while `helper` is `(result f64)` (`.build/probe/fbare.mn`); the forward call NODE is a FREE VAR (helper's generalized scheme instantiates a fresh result var that helper's later-inferred body never unifies — instantiate freshens, disconnecting caller's β' from helper's resolved β), and `repr_of(free var) = RI32` — the word floor. Even arith's `unify(lh,rh)` (infer_binop BKArith, infer.mn:2006) can't rescue it: BOTH `float_of_int(x)` and `helper(x)` are floored/free at that point, so the multiply's f64-ness comes only from the STRUCTURAL LConvert arm (emit), leaving the helper operand i32. CONTROL: `.build/probe/fback.mn` (helper FIRST) assembles CLEAN — pure order-dependence. THE DRIVER: `infer_program` (infer.mn:147) does `pre_register_decls` then `infer_stmt_list` in SOURCE order; `pre_register_fn_sig` (infer.mn:187) mints a FRESH result var and IGNORES the declared return annotation (`pre_register_stmt`, infer.mn:160, destructures `_ret_ann`). TWO FIX LEVERS: (1) ULTIMATE — infer in DEPENDENCY order (topo-sort the call graph; a leaf callee like helper generalizes to `Int -> Float` BEFORE its caller instantiates it → concrete result, no free var). (2) partial/precedented — `pre_register_fn_sig` reads the DECLARED return when annotated (Carried-Truth: read the declared intent, don't re-derive a fresh var), + annotate the forward-ref float fns `-> Float` (the float_render_positive `: Float` precedent). Lever 1 needs no annotations; lever 2 is the seed-catches-up sequencing. **IMPL ANCHORS (scoped 2026-07-08, so the fresh context BUILDS not re-scopes): ZERO top-level lets in the wheel (`grep -c '^let '` src/lib = 0) → reordering fns is SAFE (types/effects pre-register, no let-ordering to respect); non-fn stmts infer to `()` (infer_stmt's TypeDefStmt/EffectDeclStmt arms). `collect_free_vars`/`free_vars_stmt`/`free_vars_expr` (lower.mn:2952-3061) is the REUSABLE AST name-collector — one namespace, infer CAN call it (forward ref, pre-registered). `infer_fn` (infer.mn:262) RE-EXTENDS the resolved scheme at exit (generalize + env_extend, ~line 400), so a callee inferred BEFORE its caller yields a CONCRETE result — the reorder IS the whole fix. The driver's topo-partition (`driver_partition_layers`, driver.mn:216) is MODULE-granularity (import deps), INERT here (the wheel self-compiles as ONE concatenated unit). THE CHANGE: in `infer_program` (infer.mn:147), sort `stmts` by fn-fn deps BEFORE `infer_stmt_list` — Kahn's: each stmt's deps = `collect_free_vars(node, [])` ∩ {defined fn names}; emit when all deps emitted; a residual cycle (mutual recursion) emits in source order (unchanged — sees fresh-var schemes, the acceptable monomorphic-recursion limitation). CONFIRM on ffwd.mn in the fast loop FIRST, then the full battery. SCALE: ~1605 top-level fns → the sort MUST be efficient (naive O(n²)-with-list-membership is too slow); use in-degree Kahn's with O(1) membership. MOST-MENTL option (Anchor 3): build the call graph as REAL GRAPH EDGES (fn handle → dep fn handles), in-degree a node field, topo-order a graph traversal — "the call graph IS graph edges," and it composes with the future IC cursor. AVOID re-inference approaches (two-pass / targeted-fixpoint): re-inferring a fn re-mints handles + re-binds nodes (idempotency + handle-growth + fixpoint-determinism risk) — the single-pass reorder infers each fn ONCE, cleanly.** HIGH BLAST-RADIUS (inference order) → verify each step against the full 45-micro/8-rung battery (Law 7). Then m3 assembles → GATE_WASM=m3.wasm march-gate --micros → march.sh --fixpoint. **CRUCIBLE to promote once green: `.build/probe/ffwd.mn` (forward-ref float call) + `fbare.mn`/`fback.mn` — the ultimate form is order-independent (Morgan: crucibles encode the ultimate form).** #1 proven-singleton (Approach B) REFUTED — abort_exit is a LIVE uninstalled default handler**; gates: `verify.sh` + `march-gate.sh --micros`)
