...
- 2026-07-17 · A READ IS A BORROW (move-vs-borrow part 1; census 615 → 583): an `if` condition / `match` scrutinee is READ to decide control, never moved, so an `own` value used inside (`if set_contains(buf, x)`) is a borrow. A borrow_depth counter on affine_ledger + borrow_enter/borrow_exit bracket the condition and scrutinee; consume no-ops inside. Cleared 32 of 37 with soundness intact (a real `(take(buf), take(buf))` double-move to two `own` params still caught). Clean m2==m3 transition, pin fbcf14f2. Residue (5): field-read (`buf.data`) + call-arg-borrow — needs a field-receiver borrow + callee-param ownership at each call arg (a borrowing param does not consume its argument)
