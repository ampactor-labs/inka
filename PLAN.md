...
## §7 · Current state (grounded 2026-07-08 — **m3 ASSEMBLES (wat2wasm 0 errors) for the first time this arc, AND PARSES — the trap has marched all the way to the FIRST INFER PERFORM. Two Carried-Truth roots closed this session: (1) the fn-result-repr REGISTRY DELETED (8e… earlier: cfbdf8e's "wheel trusts the floored inference type" was REFUTED — the `$w.f64` USE node proved `repr_of(lookup_ty)` holds Float LIVE; the registry was a §2 side-ledger AND inert via the ev-seam; deleted whole, every call/ft width now reads `repr_of(lookup_ty(call_node))` live; m3 9→1 errors). (2) `float_of_int`/`float_to_int` TYPED AS ENV PRIMITIVES (8e9f7f3): they were recognized ONLY at lower (→ f64.convert_i32_s / i32.trunc_f64_s), UNTYPED at inference → 175 E_MissingVariable → a FREE VAR that cannot force `float_of_int(n) * x` to Float, so x floored to i32 and the product emitted `f64.mul [f64, i32]` (the standing score_one_position error). `register_primitives()` in infer_program registers `float_of_int : Int→Float`, `float_to_int : Float→Int` (pure, monomorphic) before pre_register → the multiply unifies its operand to Float. m3 wat2wasm 1→0 errors, E_MissingVariable 175→148 (the rest are OTHER recovered undefined names, pre-existing), rungs 8/0, micros-through-m2 45/0 (NO regression). THE SCORE ERROR WAS NOT A FORWARD-REF FLOOR (that framing was a partial picture — the multiply WOULD resolve proximity once float_of_int is Float). A dependency-order inference reorder (Hβ.infer.scc-ordered-walk) WAS built + verified (fbare/lit/chain forward-ref micros fixed, 45/0 no regression) then REVERTED: not needed for m3 (float_of_int typing fixes the site), and the naive `driver_partition_layers` peel is O(n²) over ~1605 decls (too slow — m3 gen ~8min). Kept as a NAMED future improvement — the ULTIMATE form is the call graph as REAL GRAPH EDGES (fn handle→dep handles, in-degree a node field, topo-order a graph traversal, composes with the IC cursor); build it ONLY when a bare-forward-float case (no forcing multiply) actually bites. **THE SOLE FRONTIER NOW — THE ev-seam / arm-internal-perform gap.** `GATE_WASM=m3.wasm march-gate --micros` = 0/8 rungs: m3 assembles + parses but TRAPS at RUNTIME compiling EVEN `fn main() = 7`. Backtrace: `ev_perform_entry ← pre_register_fn_sig ← op_each_handler_yield ← iterate_from ← iterate ← each ← infer_program`; memory fault at 0xfffffff8 (= entry−8, a genuine evidence MISS, entry==0). `pre_register_decls = each(pre_register_stmt, stmts)`; pre_register_stmt performs Graph/Env (mint/graph_fresh_ty/graph_bind/env_extend) from INSIDE each's Iterate-yield arm, and that perform's evidence does NOT reach the outer `infer_ctx`/`fresh_for_inference` (installed OUTSIDE each). m2 (seed-compiled) threads it correctly; m3 (m2-compiled) does not. **PINNED by emit-diff of `pre_register_fn_sig` (m2.wat:30565 vs m3.wat:94639) — THE CONCRETE DIVERGENCE:** before each effectful call, m2 builds an `$sst_` clone = copy the callee's closure record (fn_ptr@0, ncap@4, captured_evs), then APPEND an evidence entry `[key, entry=ev_lookup(caller __state, key), 0]`, and pass the EXTENDED clone as `__state`; the callee's `ev_perform_entry(__state, __state, 8, key)` then finds its evidence. m3 passes the RAW callee global as `__state` — **ZERO `$sst_` extension** — so the callee's `ev_perform_entry` finds entry==0 → the OOB. **This confirms the long-banked root: the WHEEL's emit (src/backends/wasm.mn call emission) LACKS the `sst_` caller-side evidence-passing convention the SEED's emit has (`sst` absent from src/ entirely; ~48 sites in the seed's emit, 0 in the wheel's). NOT the LMakeClosure handler-stack hypothesis — the divergence is the caller-side `__state` extension at EVERY effectful call.** THE FIX = build the evidence-passing call convention in the wheel's emit: at each effectful call site, lower DERIVES the callee's needed `[ename→entry]` set (which effects the callee performs, read from its row live), and emit CLONES the callee record + APPENDS `[key, ev_lookup(caller __state, key)]` per needed effect, passing the extended clone as `__state` — mirror the seed's sst_ shape (hm-seed.wat, the `sst_` family). Design-first: ONE projection both the ft-vector and the call-emit read (the original two-consumer seam dissolves by construction). first-light = diff(m3,m4) empty AND battery green THROUGH m3 — both gated on this seam. **CRUCIBLES to promote once green: `.build/probe/ffwd.mn`/`fbare.mn`/`fback.mn` (forward-ref float, for the reorder) + `.build/probe/lit.mn` (float-literal forward multiply).** #1 proven-singleton (Approach B) REFUTED — abort_exit is a LIVE uninstalled default handler**; gates: `verify.sh` + `march-gate.sh --micros`)
