...
## §7 · Current state (grounded 2026-07-07 — **m3 ASSEMBLY marching. Reachability-consistency root CLOSED (dcac3b8); noreturn CLOSED (e1899f9). The f64 root's MECHANISM is CLOSED (cfbdf8e): the wheel's emit trusted the FLOORED inference type for widths; the seed predicts them STRUCTURALLY (emit_expr_is_f64) and never floors. The wheel now mirrors it — `tail_expr_repr` gains the reliable leaves (LConvert IntToFloat→f64, arith binop→either-operand-f64, direct call→a fn-result-repr registry), `emit_binop_for`/`vec_push`/`call_ft_name` read the structural repr, and a monotone-fixpoint pre-pass seeds the registry. MEASURED: `float_of_int(x)+float_of_int(x)`→f64.add (was i32.add), the forward-ref `float_of_int(x)*weight()`→f64.mul + weight's call typed (result f64). rungs 8/0, micros-through-m2 45/0, m2 assembles — no regression. m3 CONFIRMED at ONE assembly error (the march at cfbdf8e): `undefined local $result.f64` in `parse_float` (lib/runtime/strings.mn:704) — `let result = parse_float_body(…)` (an f64-returning CALL) used in `float_of_int(sign) * result` (f64.mul). The decl+store floor to `$result` (i32) while the use is `$result.f64` (f64). ROOT: `walk_locals`' `llet_repr` → `tail_expr_repr(LCall)` → `lookup_fn_result` FLOORS in the walk_locals context (the EV-SEAM — the SAME read WORKS in call_ft_name, so weight()'s call ft is (result f64); context-dependent evidence-drop). THE FIX (fresh, ev-seam-INDEPENDENT): a per-fn LOCAL LEDGER (the seed's emit_fn_local_is_f64) — each local name's width = the JOIN over ALL occurrences (the LLet binding + every LLocal use) via `repr_of(lookup_ty)` (which threads reliably, unlike the registry perform), so the f64 USE rescues the floored decl. Populate at emit_one_fn (has the whole body; ev-seam-free); the decl (walk_locals:1688), the LLet STORE, and the LLocal use (wasm.mn:2472) all read the ledger → one width per name. Reads in emit_expr thread like string_offset. Then m3 assembles → GATE_WASM=m3.wasm march-gate --micros → march.sh --fixpoint. Promote `.build/probe/f64repro-*.mn` + `f64iso.mn` to ratcheted crucibles once green (Morgan 2026-07-07: tests encode the ULTIMATE form). #1 proven-singleton (Approach B) REFUTED — abort_exit is a LIVE uninstalled default handler**; gates: `verify.sh` + `march-gate.sh --micros`)
