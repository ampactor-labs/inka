...
- generated by: `bash tools/march.sh` (the ratchet loop; prior pin
  4b7f998f… — the region-tag-truth wheel, whose entry follows)

Prior pin's provenance:

- source: the region tag speaks truth — THE BOARD IS WHOLE (2026-07-16,
  seventh pin of the day; frontier 47/0, proof-exactness 9/9, census
  3,002). The last red class was a silent wrong-offset read: the region
  tracker's tag record crossed the type-erased bucket scan and
  `.region_id` resolved to OFFSET 0 — the handle field posed as the
  region id (pinned in the binary: `(local.get $tag) (i32.load offset=0)`
  in $op_region_tracker_check_escape; every diagnostic's "region N" was
  the value's own handle, monotone with line number), so every fn whose
  body IS a bare tuple/record literal flagged a false escape — the
  wheel's own 23, the shadow's 5, the two scheduled-fixture reds, one
  class. The tag is now the TUPLE (install_span, region_id), destructured
  positionally — provable with no type flow, the render_audit precedent —
  and the dead handle field (zero readers, it duplicated the bucket key)
  is deleted. The diagnostic gains the region's install span. The live
  check keeps one true meaning for the layer-3 arena build: regions open
  at Alloc-absorbing installs (Hβ.infer.region-on-tee-alloc-absorb), the
  fn return TRANSFERS the value into the caller's region
  (Hβ.own.region-return-transfer), and E_RegionEscape then fires only on
  a value whose region genuinely died holding it. Measured: the int-tuple
  repro 1 -> 0; wheel self-compile escapes 23 -> 0; shadow 66 -> 61
  lines, fingerprint re-pinned f725d0f6…; frontier 45/2 -> 47/0 at the
  DEFAULT boot tier. Battery: 70/70 micros, march ✓✓ FIXED POINT m2 == m3
  (479,560 lines), IDE re-packed + shim verified.
- boot/mentl.wasm  sha256 4b7f998fc991467b54d6610b297bd2cf91d223bef04165bf6607009a4b4bcab1
- its .wat (m2 == m3 asserted) sha256 47eb0669198c4b27b5e02f7301d7e69b352d84da71cb8328422ff6312f94890f
- generated by: `bash tools/march.sh` (the ratchet loop; prior pin
  701c7024… — the executable-gate wheel, whose entry follows)
