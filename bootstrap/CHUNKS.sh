...
#
# Sourced by bootstrap/build.sh (assembles bootstrap/mentl.wat) and
...
CHUNKS=(
  ...
  #      reason; canonical algorithms src/infer.mn:1818-1998 + spec 04
  ...
  "bootstrap/src/infer/main.wat"         # Tier 8 (uses walk_stmt.infer_program; Hβ.infer §8.1 + §10.3 + §13.3 #10 — closes the cascade; pipeline-stage boundary $mentl_infer; $sys_main retrofit deferred to peer handle Hβ.infer.pipeline-wire pending Hβ.lower)
  ...
  #      tag region 300-334 (per Hβ-lower-substrate.md §2; src/lower.mn:97-150
  ...
  # CASCADE CLOSED: 11/11 chunks live; $mentl_lower pipeline-stage boundary
  # named at chunk #11 main.wat. Cursor advances to Hβ.infer.pipeline-wire
  # (peer-handle commit retrofitting $sys_main to chain
  # parsed |> $mentl_infer |> $mentl_lower |> $emit_program) + Hβ.emit
  # substrate growth.
  ...
  "bootstrap/src/lower/lowfn.wat"        # Tier 4 (uses $make_record + $record_get + $record_set from record.wat; Phase C.1 — LowFn record tag 350; 6 exports — $lowfn_make + 5 accessors; row as first-class field per src/lower.mn canonical)
  "bootstrap/src/lower/lowpat.wat"       # Tier 4 (uses $make_record + $record_get + $record_set + $tag_of from record.wat; Phase C.2 — 9 LowPat variants tags 360-368 + LPArm tag 369; 27 exports; Bool→LPCon per drift-6; no guard field per SYNTAX.md)
  ...
  "bootstrap/src/lower/main.wat"          # Tier 9 (uses lower/walk_stmt $lower_stmt_list; Hβ.lower §4.3 + §10.3 + §13 + §12.3 #11 — closes the cascade; pipeline-stage boundary $mentl_lower per Hβ-bootstrap §1.15 + §10.3 — symmetric to $mentl_infer; $sys_main retrofit deferred to peer handle Hβ.infer.pipeline-wire per Lock #4 two-stage cascade-closure-then-peer-handle pattern)

  # ── Layer 6: Emitter (LowExpr-consuming substrate per Hβ-emit-substrate.md §7.2) ──
  "bootstrap/src/emit/state.wat"          # Tier 4 (uses $alloc + list + record + str_eq + str_len; Hβ-emit §3 + §3.5 + §5.1 + §7.1 + §11.3 — emit-time funcref-table accumulator + body-context captures+evidence pair + string-intern table; mirror of wheel src/backends/wasm.mn:117-128 string_table + 444-475 collect_fn_names + 945-978 emit_fn_body + 960-961 set_body_captures/_evidence; 14 exports — $emit_init $emit_funcref_register/_lookup/_count/_at $emit_set_body_context $emit_body_captures_count $emit_body_evidence/_len $emit_string_intern/_lookup $emit_string_table_count/_at $emit_fn_reset; named follow-ups: evidence-slot-naming, string-intern-pre-pass)
  ...
  "bootstrap/src/emit_data.wat"           # Tier 5 (shared WAT-token data segments for emit_infra + LowExpr emit arms)
  "bootstrap/src/emit_infra.wat"          # Tier 5 (EmitMemory/WAT output primitives consumed by all emit arms)
  ...
  "bootstrap/src/emit/emit_call.wat"      # Tier 6 (THE GRADIENT CASH-OUT SITE per Hβ-emit §2.4 — row inference's >95% monomorphic claim cashes out at WAT here per SUBSTRATE.md §I "Duty of Inference is Reification"; 6 exports — $emit_lcall (tag 308 W7 closure-call convention via call_indirect through closure's fn-ptr field — H1 evidence reification, NOT vtable indirection) + $emit_ltailcall (tag 309 same shape with return_call_indirect — tail-resumptive ~85%) + $emit_lbinop (tag 306 dispatches on BinOp ADT i32 sentinel 140-153) + $emit_lunaryop (tag 307 dispatches on UnaryOp ADT i32 sentinel 160-179 — UNeg=160 UNot=161 per src/types.mn UnaryOp ADT; substrate-honest integer-tag, NOT string-keyed) + $emit_lsuspend (tag 325 H1.6 polymorphic minority — transient closure record allocated via $emit_alloc_dyn EmitMemory swap surface + capture-copy runtime loop + ev_slot stores + call_indirect — THE LOAD-BEARING DRIFT 1 REFUSAL ARM, fn_idx is FIELD on closure record) + $emit_lindex (tag 320 dispatches on is_str — \$list_index for non-string, \$byte_at for string — substrate-correct vs wheel's flat-arithmetic drift); chunk-private \$ec6_emit_* byte-emission helpers; static data 1856 "alloc_size" + 2244 "state_tmp" length-prefixed; RETROFITS \$emit_lexpr with 6 new tag arms per Hβ.emit.lexpr-dispatch-extension; named follow-ups: lexpr-dispatch-extension)
  "bootstrap/src/emit/emit_f64_ops.wat"   # Tier 6 (uses emit_byte from emit_infra; the f64 emit family — the 13 byte-emission fns for (f64.add/sub/mul/div/eq/ne/lt/gt/le/ge/neg), (f64.convert_i32_s), (i32.trunc_f64_s), dispatched by $ec6_emit_f64_binop_op in emit_call.wat. Registered here to close the STAGE-0 gap: the file was authored but the manifest edit lived outside bootstrap/src, so the diff-scoped save dropped it.)
  "bootstrap/src/emit/emit_handler.wat"   # Tier 6 (uses lower/lexpr accessors + emit/state \$emit_body_captures_count + emit_const \$ec_emit_local_set_dollar + emit_local \$el_emit_local_get_state/_i32_load_offset + emit_call \$ec6_emit_args/_call_indirect_ftN + emit_infra \$emit_byte/_int/_str/_i32_const; Hβ-emit §2.5 + §3 + §5.1 + §7.1 + §11.3 — handler family where ~> and <~ become physical at WAT; 7 exports — \$emit_llet (tag 304 sub-emit value + "(local.set \$<name>)") + \$emit_ldeclarefn (tag 313 expression-position no-op marker per H1.4; module-level fn body emitted by chunk #9 main.wat orchestrator) + \$emit_lhandlewith (tag 329 ~> verb tail-resumptive inert seed) + \$emit_lhandle (tag 332 same inert substrate; arms emitted separately at module-emit time) + \$emit_lfeedback (tag 330 <~ verb state-machine lowering per LF walkthrough §1.5: load-prior → emit body → tee-current → store-current → reload-current; per-site state global \$s<h> + locals \$__fb_prev_<h> + \$__fb_<h>) + \$emit_lperform (tag 331 args + "(call \$op_<name>)" per H1.4 single-handler-per-op naming, monomorphic direct-call form) + \$emit_levperform (tag 333 H1 evidence-reification — load fn_idx from \$__state at offset 8+4*body_capture_count+4*slot, push __state+args, call_indirect — DRIFT 1 REFUSAL LOAD-BEARING ARM, fn_idx is FIELD not vtable); chunk-private \$ec7_emit_* byte-emission helpers; RETROFITS \$emit_lexpr with 7 new tag arms; LMakeClosure (tag 311) + LMakeContinuation (tag 312) deferred to named peer Hβ.emit.handler-fnref-substrate which depends on Hβ.lower.lowfn-substrate; named follow-ups: lexpr-dispatch-extension, lowfn-substrate, handler-fnref-substrate)
  "bootstrap/src/emit/main.wat"           # Tier 9 (uses \$emit_lexpr from emit_const + \$len/\$list_index from runtime/list; Hβ-emit §4 + §10.3 + §11.3 — pipeline-stage boundary closing the Hβ.emit cascade; 2 exports — \$emit_lowir_program (algorithmic-core orchestrator iterating LowExpr list via \$list_index + \$emit_lexpr; Ω.3 buffer-counter substrate; loop-only — emission side-effect lives in arms) + \$mentl_emit (pipeline-stage entry per Hβ-bootstrap §1.15 \$inka_<verb> convention — Lock #2 two-symbol pattern third instance after \$mentl_infer + \$mentl_lower); Lock #3 no result — emission is side-effect on \$out_base/\$out_pos buffer; Lock #4 \$sys_main retrofit is named peer Hβ.infer.pipeline-wire NOT touched here; emit_dispatcher.wat absorbed into emit_const.wat per walk_call.wat precedent — first chunk needing recursion introduces partial dispatcher, subsequent chunks retrofit; named follow-ups: pipeline-wire, lowfn-substrate, handler-fnref-substrate, lmatch-pattern-compile, memory-arena-handler, memory-gc-handler, module-wrap)
