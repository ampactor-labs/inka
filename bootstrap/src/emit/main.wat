  ...
  (func $emit_fn_body (param $fn_r i32)
    ...
    ;; then the ledger entries past the params. The emission IS the
    ;; locals walk — a use-site the preamble misses is unrepresentable.
    ;; Replaces the parallel decl-walk family (let/alloc/hstate/match/pat)
    ;; that diverged from the emit dispatcher four separate times.
    ;; Pre-register the standard scratch names $emit_standard_locals
    ;; already declared — these five are the ONLY standard names that
    ;; exist as length-prefixed strs, so they are the only ones a body
    ;; projection can pass (emit_lmakeclosure allocs into state_tmp);
    ;; the dump must not re-declare them.
    (drop (call $emit_fn_local_check (i32.const 2244)))   ;; state_tmp
    (drop (call $emit_fn_local_check (i32.const 1568)))   ;; variant_tmp
    (drop (call $emit_fn_local_check (i32.const 1552)))   ;; record_tmp
    (drop (call $emit_fn_local_check (i32.const 1536)))   ;; tuple_tmp
    (drop (call $emit_fn_local_check (i32.const 1856)))   ;; alloc_size
    (local.set $locals_start (global.get $emit_fn_locals_len_g))
