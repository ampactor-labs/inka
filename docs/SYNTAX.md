...
**The declared depth is not yet honored** — `delay(3)` emits the same one-slot register `delay(1)` does (measured: `delay(0)`, `delay(1)` and `delay(3)` return identical values). The depth read, and with it an N-slot ring of priors, is `Hβ.dataflow.feedback-delay-depth-unread`; the zero-delay refusal above becomes one arm of that read.
...
| `E_ZeroDelayFeedback` | `x <~ delay(0)` — a cycle with no delay: the prior would be the value being computed. ARMED (refuses the executable); decidable at a literal depth only, both the `delay` and `Delay` spellings | `MaybeIncorrect` | raise the delay to at least 1 |
