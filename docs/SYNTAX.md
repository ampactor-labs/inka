...
**The declared depth IS the line's depth (real, 2026-08-12).** `delay(N)` carries N priors: the site's state is an N-slot line, the prior the cycle reads is its OLDEST slot — y[n−N] — and each tick advances the line by one. `delay(1)` is the single register it always was; `delay(3)` is genuinely three deep, measured by the same recurrence driven six times answering 6 under `delay(1)` and 2 under `delay(3)`. The slots are declared, never allocated, so depth costs nothing per tick and the `!Alloc` row survives at any N.
