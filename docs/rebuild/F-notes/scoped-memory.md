# The Inka Memory Superpower: Ownership + Scoped Allocators
...
But we realized we **don't**. We already have everything we need built into Inka's core philosophy.

## The Superpower: `Alloc` as an Effect + Ownership

In Inka, `Alloc` is not a magic compiler keyword—**it is an Effect.**
...
**This is where Inka's Ownership system takes over.**

Inka maintains a borrow graph (via the `report_ownership` compiler pass). If `find_similar_name` returns `similar`, the compiler statically knows that `similar` points to memory "owned" by `temp_arena`. 
...
WASM threads usually require a global allocator protected by an atomic mutex—meaning threads bottleneck each other every time they allocate. In Inka, if `Alloc` is handled via thread-local memory effects by default, concurrency scales infinitely with zero locking out of the box.

### 3. User-Space Memory Engineering
Inka doesn't need to ship a GC inside its WASM binaries. There is no heavy runtime. Developers have complete native control over memory strategies—building their own pool allocators, ring buffers, generational arenas, or WebGL shared memory handlers—by simply writing new handlers for the `Alloc` effect, all statically verified by the ownership system. 
