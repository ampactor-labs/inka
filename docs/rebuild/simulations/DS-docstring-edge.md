...
**Gap.** The lexer (lexer.ka:167-173) emits `TDocComment(content)`
tokens. The parser (parser.ka:75, 147) recognizes them. But the
parser does NOT attach the docstring to the following declaration.
The `FnStmt` (types.ka:503-504) has no docstring field. The typed
...
`TDocComment(_)` appears in the token classification (parser.ka:75)
and the token display function (parser.ka:147). But the parser's
...
| Peer | Surface | Load |
|---|---|---|
| DS.1 | `Documented(String, Stmt)` variant + `DocstringReason` | Light (~5L types.ka) |
| DS.2 | Parser docstring attachment | Moderate (~30L parser.ka) |
| DS.3 | Inference docstring threading to Reason | Moderate (~20L infer.ka) |
...
- **Docstrings as comments.** `//` comments are discarded. `///`
  ...
  (lexer.ka:166-173). DS completes the pipeline.
