...
to `Closed(A - B) = Closed(A)` when `B ⊄ A` (effects.ka:150-154). The
...
**Trace.** `std/compiler/parser.ka:418-447` builds `effs : List<(EffName, negated: Bool)>` per fn signature. `std/compiler/infer.ka:376-387` (`build_declared_from`) iterates the list and builds an `EffRow` via `union_row` / `inter_row(_, neg_row(_))`. `std/compiler/effects.ka:134-156` (`normalize_inter`) reduces `Closed(A) & !Closed(B)` to `Closed(A - B)` — algebraically correct, but after this reduction the pair `(Name, negated=true)` is no longer recoverable from the normalized row.
...
**What's preserved today.** The `FnStmt(name, params, ret_ann, effs, body_node)` AST node at `types.ka` holds `effs` in authored form. It's reachable by traversing the AST. Nothing in the compiler currently threads it through to Mentl / audit / hover handlers.
...
**Trace.** The grammar accepts `with <EffectName> [+ <EffectName> ...]` (parser.ka:418). Each `EffectName` is either `ENamed(String)` or `EParameterized(String, args)`. The parser has no path for "alias that expands to a row expression."
...
**Load.** Moderate-heavy. New top-level decl syntax (parser.ka). New SchemeKind variant (types.ka). New resolution branch in `build_declared_from` (infer.ka). `cache.ka` serialization of `CapabilityScheme`. `show_scheme` support.
...
- **`fn realtime_map(f: fn(A) -> B with E + !Alloc, xs)`** in a new `std/realtime.ka` module — explicit DSP combinator that restricts callbacks. Doesn't touch prelude.
- **`fn map(f, xs)` in prelude.ka** — adding `!Diagnostic` to callback rows would restrict what callers can pass. High risk, high value: maps that never propagate reporting.
...
**Mentl's choice: new `std/realtime.ka` module.** Lowest-risk, highest-exemplar-clarity. Prelude stays general-purpose; realtime ops are a focused capability-typed library. The exemplar lives in a module whose existence proves primitive #4's polymorphic expressive power.
...
### §4.4 Backends / emit (wasm.ka)

No change. Effect rows are compile-time only; emitter works from the lowered IR.

### §4.5 Mentl / audit / hover handlers (mentl.ka, future audit.ka)
...
| β | — | — | — | — | — | — | AT11-12 + new std/realtime.ka ~80L |
