...
Scan of `std/compiler/*.ka` + `std/compiler/backends/*.ka` + `std/prelude.ka`
...
|---|-----------|--------|----------|
| 1 | SubstGraph + Env | ✓ used | graph.ka, infer.ka, driver.ka |
| 2 | Handlers with resume discipline | ✓ used (OneShot only) | ~20 top-level handlers in clock.ka / graph.ka / infer.ka etc. |
| 2a | MultiShot resume | 1 decl (mentl.ka:94 `enumerate_inhabitants`) | Mentl's op; never actually invoked from compiler source |
| 2b | Parameterized effects | **0 uses** | no `effect Name(arg) { ... }` in compiler |
| 3a | `\|>` sequential | ✓ used | main.ka / pipeline.ka pipelines |
| 3b | `<\|` divergent | **0 body uses** | only in comments, ADT decls (PipeKind variants) |
| 3c | `><` parallel | **0 body uses** | only in comments, ADT decls |
| 3d | `~>` handler-attach | ✓ used | main.ka:42-46, pipeline.ka:64-82 (9 handler-chain sites) |
| 3e | `<~` feedback | **0 body uses** | only in comments, ADT decls |
| 4a | Effect row `+` | ✓ used | 27 fn signatures with `with X + Y` |
| 4b | `!E` negation | **0 uses** | 3 hits are string-literals in mentl.ka rendering |
...
types.ka and use them throughout:
```
