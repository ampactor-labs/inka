...
We also updated `unify_row_canonical` in `effects.ka` to properly unify `EfNeg` nodes, preventing infinite loops when higher-order functions unify their universe-minus capability constraints.
...
Because `test_mutate_freeze` resides in `effects.ka`, the Inka compiler must successfully parse and statically verify it. This proves that `!Mutate` (FV.4 ownership marker) is now fully native to the architecture.
