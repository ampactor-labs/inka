...
(infer.ka:259-273). This fires when the developer declares MORE
...
### Effect row delta (infer.ka:233-297)

After `inf_exit_fn()` binds the accumulated body row, and after
`declared_effs` is read, infer.ka computes the subsumption check:
...
### Ownership delta (own.ka:411-434)
...
| Peer | Surface | Load |
|---|---|---|
| GR.1 | `AnnotationSuggestion` ADT + `GradientQuery` effect | Light (~20L types.ka) |
| GR.2 | Effect-row delta computation in gradient handler | Moderate (~40L query.ka or gradient.ka) |
| GR.3 | Ownership delta computation | Light (~20L, depends on OW) |
| GR.4 | Mentl Teach arm integration | Moderate (~30L mentl.ka) |
