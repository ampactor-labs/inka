...
Parser needs PRecord(fields) — already in types.ka at
...
partially implemented in effects.ka's row-unification for EffRow.
...
Today PRecord is in types.ka but never lowered. H2 adds the
lowering.
...
- **Row polymorphism unification on TRecord/TRecordOpen.** This
  is ACTUAL WORK — the effects.ka row-unification pattern applied
...
- ~5 files touched: types.ka (MakeRecordExpr variant), parser.ka
  (type/expr/pattern parse arms + disambiguation), infer.ka
  (MakeRecordExpr + PRecord arms + TRecord/TRecordOpen
  unification), lower.ka (MakeRecordExpr lowering + PRecord
  match lowering), backends/wasm.ka (LMakeRecord emission +
  5 match sites).
