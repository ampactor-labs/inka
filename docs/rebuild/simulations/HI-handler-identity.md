...
diagnostics_handler installed at pipeline.ka:42."
...
at pipeline.ka:38." The handler identity — which IS the capability
stack's layer naming — doesn't reach the downstream surface.

The `handler_stack` in `infer_ctx` (infer.ka:45-47, 56-60) already
...
### Handler installation (infer.ka:524-596)
...
For `~>` tee sites (infer.ka:821-837):
```
...
### Handler declaration (infer.ka:179-180)
...
### PerformExpr (infer.ka:519-522, 744-768)
...
### Diagnostics (effects.ka:373-389, own.ka:35-41)
...
| Peer | Surface | Load |
|---|---|---|
| HI.1 | Fix handler_stack tags (authored names) | Moderate (~20L infer.ka) |
| HI.2 | `handler_provider` query op | Light (~15L query.ka or infer.ka) |
