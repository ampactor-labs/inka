...
character edit to `std/compiler/infer.ka` and the LSP re-checks the
...
Salsa 3.0 + overlay pattern in `graph.ka`. What pends is the driver.*

---

## The scenario

Morgan opens `std/compiler/infer.ka`. The whole project is checked
...
... full recompile of all 16 .ka files ...
Done in 1.4s.
```
...
incremental check:
  cache/.kai hashes:
    types.ka         — hash unchanged, env loaded from cache (3ms)
    graph.ka         — hash unchanged, env loaded from cache (4ms)
    effects.ka       — hash unchanged, env loaded from cache (3ms)
    infer.ka         — hash CHANGED, re-infer (62ms)
    lower.ka         — public env unchanged in deps, env loaded (5ms)
...
- **`graph.ka`'s shape is Salsa 3.0.** Flat array + epoch +
...
- **Content-hash keying** — `runtime/strings.ka` has the byte-level
...
A serialized per-module env. One file per `.ka` source.
...
- The Scheme's TVar handles refer to B's overlay handles. When A's
  ...
  `current_overlay_idx` discipline already in graph.ka).
...
When the LSP receives `textDocument/didChange` for `infer.ka`:

1. Hash the new buffer content.
2. If hash unchanged from last check → no-op (typo/whitespace fix).
3. Else: invalidate `infer.ka.kai`. Walk import-graph downstream:
   any module that imports `infer.ka` invalidates too (their
...
<50ms. A core-module edit (touching `types.ka`) cascades but the
...
source bytes. File I/O via existing `runtime/io.ka` primitives.
...
- ~400-600 lines across 3 new modules:
  - `std/compiler/cache.ka` — KaiFile, hash, serialize, deserialize
  - `std/compiler/driver.ka` — module DAG, topo sort, per-module
    invocation, cache invalidation walk
  - LSP handler arms wiring to driver

- Modifications to existing modules:
  - `graph.ka` — chase function walks overlays for handle resolution
  - `pipeline.ka` — `compile` and `check` route through driver
  - `main.ka` — CLI entry to `inka check --watch` and similar

- One coordinated commit-cluster (~3-5 commits): cache.ka first
  (data type + serialization, no driver), driver.ka second (uses
...
3. **Leaf-module edit.** Change a one-line in a leaf (e.g., `main.ka`
   that no other module imports). Only that module re-infers;
   everything else loads from cache. Diagnostic in <100ms.

4. **Core-module edit.** Change `types.ka`. Every module that
...
5. **Public-surface unchanged after dep change.** Edit a private
   helper in `effects.ka`. v1: `infer.ka` still re-infers
   (conservative). v2 (future): `infer.ka` cache stays valid
   because `effects.ka`'s exported_entries hash didn't change.
...
Recommended: land `cache.ka` first (data + serialization, no
driver), then `driver.ka` (uses cache, no LSP), then LSP
integration third. Each commit is independently useful: cache.ka
alone enables `inka check` to write caches even without
invalidation logic; driver.ka enables warm-path checks; LSP wires
the editor.
