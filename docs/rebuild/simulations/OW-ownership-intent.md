...
`consume(name, span)`. The `affine_ledger` handler (own.ka:30-93)
tracks consumed names; a double-consume emits `E_OwnershipViolation`.

**Gap.** The diagnostic at own.ka:35-41 reads:
...
Similarly, `ref` escape diagnostics at own.ka:351-355 read:
...
1. **No ownership provenance on Consume ops.** When `affine_ledger`
   ...
   or INFERRED as `Own` by `infer_ownership` (own.ka:417-434). The
...
### Inference (infer.ka:200-304)

`infer_fn` mints param handles and extends env with the fn's
scheme. `check_consume_at_use` (infer.ka:705-711) performs
...
### Ownership inference (own.ka:417-434)
...
**Load.** Moderate. TParam gains a field (types.ka). Every TParam
...
### Mentl / hover
`show_tparam` (types.ka:793-798) gains authored/resolved display.
...
**AT-OW4.** `ref` escape diagnostic at own.ka:351 distinguishes
...
| Peer | Surface | Load |
|---|---|---|
| OW.1 | Reason-edge provenance for ownership on env entries | Light (~10L types.ka + ~5L infer.ka) |
| OW.2 | Dual-field TParam (authored + resolved) | Moderate (~20L types.ka + ~30L own.ka) |
| OW.3 | Diagnostic message upgrade (authored vs inferred) | Moderate (~25L own.ka) |
| OW.4 | Hover/Mentl authored ownership surfacing | Light (~15L types.ka) |
