...
In `std/compiler/infer.ka:187-189` (`RefineStmt` handling):
...
| Peer | Surface | Load |
|---|---|---|
| RN.1 | `TAlias` variant + infer `RefineStmt` wrapping | Light (~15L types.ka + ~5L infer.ka) |
| RN.2 | Unification alias preservation + `show_type` rendering | Moderate (~30L infer.ka + ~10L types.ka) |
| RN.3 | Verify diagnostic upgrade (alias + sub-predicate) | Moderate (~40L verify.ka) |
| RN.4 | Cache serialization round-trip | Light (~15L cache.ka) |
