...
and every unification step records a Reason (types.ka:231-251). The
...
**Gap.** The current Reason vocabulary (types.ka:231-251) has 18
...
|---|---|---|
| infer.ka env_extend for FnStmt | `Declared(name)` | Names the function |
| infer.ka env_extend for RefineStmt | `Declared(name)` | Names the refinement alias |
| infer.ka VarRef | `VarLookup(name, reason)` | Names the variable |
| infer.ka FnReturn | `FnReturn(name, reason)` | Names the function |
| infer.ka FnParam | `FnParam(name, idx, reason)` | Names the param |
| infer.ka LetBinding | `LetBinding(name, reason)` | Names the binding |
| infer.ka MatchBranch | `MatchBranch(scrutinee, arm)` | Links both sides |
| infer.ka UnifyFailed | `UnifyFailed(a, b)` | Carries the types |
| effects.ka Located | `Located(span, reason)` | Site-annotated |
| infer.ka Refinement | `Refinement(pred, pred)` | Carries predicates |

### Grade B (partial-intent — could be improved)

| Site | Current Reason | Missing intent |
|---|---|---|
| infer.ka:204 | `FnReturn(name, Fresh(0))` | The `Fresh(0)` says nothing; should be `Inferred("return of " ++ name)` |
| infer.ka:205 | `Inferred("fn effects")` | Should be `Inferred("effects of '" ++ name ++ "'")` |
| infer.ka:394-398 | `Inferred("int literal")` etc. | Could carry the literal value: `Inferred("int literal 42")` |
| infer.ka:437 | `Inferred("if condition")` | Could name the enclosing fn |
| infer.ka:447 | `Inferred("block result")` | Could name the enclosing fn |

### Grade C (low-intent — mechanism-speak)

| Site | Current Reason | What should it say |
|---|---|---|
| infer.ka:790-799 | `Inferred("|> return")` etc. | Should name the pipe verb: `Inferred("forward pipe (|>) at line N")` |
| infer.ka:718-719 | `Inferred("call return")`, `Inferred("call effects")` | Should name the callee: `Inferred("return of call to '" ++ callee_name ++ "'")` |
| infer.ka:726-727 | `Inferred("expected fn")`, `Inferred("expected fn type")` | Should name the call site: `Inferred("expected type of '" ++ callee_name ++ "'")` |
| infer.ka:460-461 | `Inferred("empty list element")`, `Inferred("empty list")` | Acceptable, but could carry span |
| graph.ka (fresh alloc) | `Fresh(handle)` | Should carry the allocation context: `FreshInContext(handle, fn_name)` |
...
### effects.ka
No change — effects.ka Reasons are already `Located`.
...
| RX.2 | New Reason variants in types.ka + show_reason | Moderate (~20L types.ka) |
| RX.3 | Grade B string enrichment (~15 sites in infer.ka) | Moderate (~30L infer.ka, 1-2L each) |
| RX.4 | Grade C structured variant replacement (~8 sites) | Moderate (~25L infer.ka + graph.ka) |
...
- **Inventing new Reason architecture.** The current Reason DAG
  (types.ka:231-251) is sound. The structure works. The problem is
