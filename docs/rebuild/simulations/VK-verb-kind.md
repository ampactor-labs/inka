...
- `|> converge` → `LCall(handle, lo_r, [lo_l])` (lower.ka:409)
- `<| diverge` → `LMakeTuple(handle, lower_diverge_branches(...))` (lower.ka:413-414)
- `>< compose` → `LMakeTuple(handle, [lo_l, lo_r])` (lower.ka:417-418)
- `~> tee` → `LHandleWith(handle, lo_l, lo_r)` (lower.ka:427-430)
- `<~ feedback` → `LFeedback(handle, lo_l, lo_r)` (lower.ka:434-435)
...
### types.ka
No change. PipeKind already defined (types.ka:398-404).
...
| Peer | Surface | Load |
|---|---|---|
| VK.1 | `pipe_topology` query op + handler arm | Light (~25L query.ka) |
