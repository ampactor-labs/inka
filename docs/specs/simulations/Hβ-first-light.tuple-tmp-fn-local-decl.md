...
| **E.5** (self-bootstrap delta) | Post-fix: `wat2wasm /tmp/mentl2.wat -o /tmp/mentl2.wasm 2>&1 \| grep -c 'undefined local variable "\$tuple_tmp"'` returns 0 (was ≥ 1 pre-fix for any wheel file that uses tuples). |
