...
> §2.3 (link.py named as the LAST non-Mentl substrate; ~200 lines
...
> partial WATs).
>
> *Claim in one sentence:* **link.py is the LAST non-Mentl substrate
...
> WASI imports, wires `_start` to `main.mn`'s compiled `main()`,
> and emits one assembled `mentl.wat` that wat2wasm compiles. Per
...
cat src/effects.mn  | wasmtime run bootstrap/mentl.wasm > /tmp/effects.wat
cat src/graph.mn    | wasmtime run bootstrap/mentl.wasm > /tmp/graph.wat
... (one invocation per .mn file)
```
...
- Module-local symbol references (e.g., `(call $list_index ...)`
  referencing lib/runtime/lists.mn's definition).
...
`mentl.wat` with all cross-module references resolved.
...
5. Wire `_start` to `main.mn`'s `main()`.
6. Emit assembled `mentl.wat`.

Per BT §3 alternative scope: ~200 lines Python.

### 0.2 Why Python (not Mentl) for this LAST non-Mentl substrate
...
> NOT Mentl semantics — the linker only resolves symbols + concatenates;
> no type-checking, no inference, no emit decisions.

Per F-retire (residue tracker §F-retire): post-first-light,
`link_handler` is a canonical Mentl handler that supersedes link.py;
...
Each invocation of the seed against one `.mn` source file produces
...
- References to other modules' exports (e.g., src/graph.mn's
  compiled output references types declared in src/types.mn).
...
module          imports
types.mn        runtime/strings, runtime/lists
effects.mn      types, runtime/strings
graph.mn        types, effects, runtime/strings
... (one row per src/*.mn + lib/**/*.mn)
```

Manifest is generated by a one-shot `bootstrap/scripts/manifest.sh`
script that scans `import` lines from each `.mn` file. Trivial
...
Module name derivation: `src/graph.mn` → `graph`; `lib/runtime/lists.mn`
→ `runtime__lists`. The `<module>` part of `<module>__<symbol>`
preserves directory structure via `__` separator (e.g.,
`graph__make_node` for src/graph.mn's $make_node OR
`runtime__lists__list_index` for lib/runtime/lists.mn's $list_index
when collision-resolved).

### 2.3 Phase 3 — deduplicate WASI imports

Each per-module WAT may declare WASI imports (`(import
"wasi_snapshot_preview1" "fd_write" ...)`). The assembled mentl.wat
...
The assembled mentl.wat needs ONE `(func $sys_main (export "_start") ...)`.
This comes from `main.mn`'s compiled `main()`:
...
main.mn's compiled function name is); link.py wraps it in `_start`
+ adds proc_exit. Per Hβ-bootstrap.md §1.15 entry-handler convention:
`mentl <verb>` resolves to `<verb>_run` handler installation; the
...
Output: one mentl.wat written to stdout (or `-o` flag-specified path).
...
Per CLAUDE.md anchor: this is the LAST non-Mentl substrate. Post-
...
MODULE_SHELL_HEADER = """;; mentl.wat — Reference Seed Compiler (assembled by link.py)
...
def main(argv: list[str]) -> int:
    ...
    # Identify main: the symbol from main.mn's module that's labeled "main".
    main_module = next((m for m in modules if m.module_name == "main"), None)
    if main_module is None:
        print("error: no main.mn in input modules", file=sys.stderr)
        return 1
...
Hβ.link is the LAST non-Mentl substrate; the eight interrogations
apply at the architectural-discipline altitude (does this work
match Mentl's substrate-honest discipline?), not at the per-line-of-
...
- **Drift 8 (string-keyed-when-structured):** symbol resolution
  ...
  Mentl-spec ModuleId equivalent; converted to `<module>__<symbol>`
...
| Go internal/vendor imports | not relevant; Mentl modules are flat |
...
```
.mn source → Hβ.lex → tokens → Hβ.parse → AST →
...
emitted. It composes them into one mentl.wat for wat2wasm.
...
- **Hβ.link.incremental** — only re-link modules that changed since
  last build; keep cached per-module compiled WAT in `.mentl/cache/`
...
- **Hβ.link.parallel** — invoke per-module seed compilation in
  ...
  up first-light-L1 cycle for large `src/*.mn` trees.
- **F-retire link** — post-first-light, `link_handler` is a canonical
  Mentl handler implementing the same protocol; link.py dissolves
...
**Sonnet via mentl-implementer is APPROPRIATE for link.py**. The
...
The mentl-implementer's brief includes:
...
Hβ.link is the LAST non-Mentl substrate in the bootstrap path. Per
...
- **No foreign-language semantic substrate.** link.py is text
  manipulation — symbol rename + concat + dedup. No Mentl concepts
  (Ty / EffRow / Handler / Scheme) appear in Python code.
- **Disposable as soon as Mentl can replace it.** Post-first-light,
...
04 / spec 05 / src/infer.mn / src/lower.mn — the projection onto
...
Mentl-bootstraps-through-Mentl — the seed's substrate is ready to
