...
Inka is built on Hindley-Milner (HM) type inference. Historically, Inka functions omitted type annotations:
```inka
...
This discovery perfectly aligns with Inka's core primitives:
...
- `FV.3.2`: 12 sites in `lexer.nx` updated with `n: ValidOffset` and `pos: ValidOffset`.
- `FV.3.4`: 56 sites in `parser.nx` and `infer.nx` updated with `span: ValidSpan`.
