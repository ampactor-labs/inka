#!/usr/bin/env python3
"""
Inka Mono — Build Script

Derives Inka Mono from JetBrains Mono by:
...
# JetBrains Mono source files → Inka Mono output files
VARIANTS = {
...
def draw_pipe_compose(pen):
    ...
    Inka's signature glyph. Two triangles meeting at center,
...
def rename_font(font, variant_style):
    """
    Replace all JetBrains Mono references with Inka Mono.
    ...
    replacements = {
        0: f"Copyright 2020 The JetBrains Mono Project Authors. "
           f"Modified 2026 Ampactor Labs (Inka Mono derivative). "
           f"Licensed under SIL Open Font License 1.1.",
        1: "Inka Mono",
        2: variant_style,
        3: f"1.0;AmpactorLabs;InkaMono-{variant_style.replace(' ', '')}",
        4: f"Inka Mono {variant_style}",
        5: "Version 1.0 — derived from JetBrains Mono 2.304 with Inka-specific ligatures",
        6: f"InkaMono-{variant_style.replace(' ', '')}",
        7: "Inka Mono",
        8: "Ampactor Labs",
        9: "Philipp Nurullin, Konstantin Bulenkov (JetBrains Mono); Ampactor Labs (Inka ligatures)",
...
def build_variant(input_path, output_path, variant_style):
    """Build one Inka Mono variant from a JetBrains Mono source."""
    print(f"\n{'='*60}")
...
def main():
    ...
    print(f'  "editor.fontFamily": "\'Inka Mono\', monospace"')
