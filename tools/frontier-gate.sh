...
run_census() {
  ...
  for spec in '|>:10' '<|:11' '><:12' '~>:13' 'anonymous:14' '<~:15' 'eta:24' 'effectful-lambda:25' 'iteration:26'; do
    q="${spec%%:*}"; line="${spec##*:}"
  ...
  [ "$ok" = 1 ] && pass "structural census: all nine shapes count their own site (|> <| >< ~> <~ anonymous eta effectful-lambda iteration)"
}
